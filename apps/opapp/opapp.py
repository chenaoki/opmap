...
def run_opapp(json_path='./param.json', raw_path=None, result_path=None):
  ...
  if menu_list["cam"] in [2,3] : rawcam.saveImage(saveDir+'/cam', skip=save_int)
  if menu_list["cam"] == 3 : makeMovie(saveDir+'/cam')
  if menu_list["cam"] == 4 : np.save(saveDir+'/cam', cam.data)
  print "done",

  if menu_list["vmem"] == 0 : return
  print "VmemMap..."
  vmem = VmemMap(rawcam)
  ...
  if menu_list["integrate"] == 0 : return
  print "Integrate..."
